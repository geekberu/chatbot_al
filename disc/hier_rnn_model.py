...
class Hier_rnn_model(object):
    def __init__(self, config, vocab_size, name_scope, dtype=tf.float32):
        ...
        encoder_emb = rnn.EmbeddingWrapper(encoder_mutil, embedding_classes=vocab_size,
        ...
        for i, bucket in enumerate(buckets):
            with tf.variable_scope(name_or_scope="Hier_RNN_encoder", reuse=True if i > 0 else None) as var_scope:
                query_output, query_state = rnn.static_rnn(encoder_emb, inputs=self.query[:bucket[0]], dtype=tf.float32)
                # output [max_len, batch_size, emb_dim]   state [num_layer, 2, batch_size, emb_dim]
                var_scope.reuse_variables()
                answer_output, answer_state = rnn.static_rnn(encoder_emb, inputs=self.answer[:bucket[1]], dtype=tf.float32)
            ...
            with tf.variable_scope(name_or_scope="Hier_RNN_context", reuse=True if i > 0 else None):
                output, state = rnn.static_rnn(context_multi, context_input, dtype=tf.float32)
